...
5. **Archive pass.** Every doc is **accurate or archived** — no third state,
   and every kept doc is routed: a router README, an entry in its router's
   README, a machine dir, or a data file. Unrouted content routes, archives,
   or becomes a papercut. New router = owner approval.
...
6. **Implement fix-now** idempotently, smallest change that satisfies the
   ...
   | features.json / durable state | yes | only if independently shipped |
   | Lock file | only if root declares deps | one per package that declares deps |
...
8. **Prove live.** Run every command this run claims works. Then acceptance:
   - **Weak-model soak** — a fresh **low-capability** agent (never the setup
     agent; doer ≠ grader) gets one starter task and only the injected docs;
     every stumble is logged as repo debt and fixed; repeat until clean.
